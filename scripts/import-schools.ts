...
async function importSchools() {
  try {
    ...
    const schoolRegex =
      /<a href="\/lectio\/(\d+)\/default\.aspx">([^<]+)<\/a>/g;
    ...
    for (const school of schools) {
      ...
      if (batchCount === 500) {
        await batch.commit();
        console.log(
          `Committed batch of ${batchCount} schools (${totalCount}/${schools.length})`
        );
        batchCount = 0;
      }
    }
    ...
    if (batchCount > 0) {
      await batch.commit();
      console.log(
        `Committed final batch of ${batchCount} schools (${totalCount}/${schools.length})`
      );
    }
